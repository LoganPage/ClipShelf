...
```powershell
& '.\.tools\dotnet\dotnet.exe' run --project windows/search-benchmarks/SearchBenchmarks.csproj -c Release -- tests
& '.\.tools\dotnet\dotnet.exe' run --project windows/search-benchmarks/SearchBenchmarks.csproj -c Release -- current
```

## 实测结果

`baseline.json` 在修改搜索实现前实际采集；`optimized-final.json` 在最终搜索实现上采集。每组有 100 或 1000 条记录，每条约 2000 字符，包含中文标题、较长正文及部分文件路径；相同查询连续执行两次。下表为第二次查询耗时，单位毫秒。
